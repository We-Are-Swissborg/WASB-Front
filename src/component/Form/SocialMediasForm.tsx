...
export default function SocialMediasForm(props: ISocialMediasForm) {
    ...
    const checkSocialMediasWithOldSocial = (newSocialMedias: FieldValues) => {
        ...
        propsData.forEach((prop) => {
            if (socialMediasRef.current && newSocialMedias)
                sameValue =
                    newSocialMedias[prop] === socialMediasRef.current[prop as keyof SocialMedias] ||
                    (newSocialMedias[prop] === '' && socialMediasRef.current[prop as keyof SocialMedias] == undefined);
                    ...
                    if (props.user?.id) {
                        ...
                        props.setUser({
                            ...
                            socialMedias: {
                                ...
                                telegram:
                                    data.telegram !== undefined ? data.telegram : props.user.socialMedias?.telegram,
                            },
                        });
    ...
    useEffect(() => {
        return () => {
            console.log('socialMediasRef', socialMediasRef.current);
    ...
    return (
        <form className="form all-form-profile" onSubmit={onSubmit}>
            <div className="div-under-form">
                {propValueSocialMedias.map((field: string, id: number) =>
                    displayInput(field as keyof SocialMedias, id),
                )}
            </div>
